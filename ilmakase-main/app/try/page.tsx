...
export default function TryPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data.preview) {
        try {
          localStorage.setItem('demoPreview', JSON.stringify(data.preview))
          localStorage.setItem('demoContents', JSON.stringify(filledItems.map((item) => item.content.trim())))
        } catch (error) {
      ...
      alert(error.message || 'AI 분석에 실패했습니다. 다시 시도해주세요.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      {/* Header */}
      <header className="p-4 flex items-center justify-between border-b border-gray-100 bg-gradient-to-r from-primary-50 to-orange-50">
        <div className="flex items-center gap-3">
          <button onClick={handleCancel} className="text-gray-500 p-1">
            <i className="fas fa-arrow-left"></i>
          </button>
          <div>
            <h1 className="text-lg font-bold text-gray-800">체험하기</h1>
            <p className="text-xs text-primary-600">로그인 없이 AI 분석을 체험해보세요</p>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 text-sm">
                <p className="font-bold text-blue-900 mb-1">체험 모드</p>
                <p className="text-blue-700 text-xs">
                  로그인 없이 AI 분석을 체험할 수 있어요. 결과는 저장되지 않습니다.
                </p>
              </div>
            </div>
          </div>
